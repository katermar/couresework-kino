...
import com.katermar.movierating.entity.*;
...
public class GeneralLogic {
    ...
    public CommandResult showFilmInfoPage(HttpServletRequest request) {
        ...
        try {
            ...
            if (session != null) {
                ...
                if (currentUser != null) {
                    Rating rate = ratingService.getRatingByUserAndFilm(currentUser.getId(), filmId);
                    request.setAttribute("seen", rate.getIsSeen());
                }
            }
        } catch (ServiceException e) {
            LOGGER.warn(e.getMessage());
    ...
    public CommandResult searchFilms(HttpServletRequest request) throws CommandException {
        ...
        List<String> searchDirectors = request.getParameterValues(Parameter.DIRECTOR) == null ?
                new ArrayList<>() : List.of(request.getParameterValues(Parameter.DIRECTOR));

        List<String> searchGenres = request.getParameterValues(Parameter.GENRE) == null ?
